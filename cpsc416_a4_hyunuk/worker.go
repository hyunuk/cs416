...
func (w *WorkerRPCHandler) Mine(args WorkerMineArgs, reply *struct{}) error {
	...
	trace.RecordAction(WorkerMine{args.Nonce, args.NumTrailingZeros, args.WorkerByte})
	if secret := w.cache.Get(CacheKey{args.Nonce, args.NumTrailingZeros}); secret != nil {
		res := WorkerResult{args.Nonce, args.NumTrailingZeros, args.WorkerByte, secret, nil}
		trace.RecordAction(res)
		res.Token = trace.GenerateToken()
		w.resultChan <- res
	} else {
...
func (w *WorkerRPCHandler) Found(args WorkerCancelArgs, reply *struct{}) error {
	...
	existed, changed := w.cache.Add(CacheEntry{args.Nonce, args.NumTrailingZeros, args.Secret})
	if existed != nil {
		trace.RecordAction(CacheRemove{existed.Nonce, existed.NumTrailingZeros, existed.Secret})
	}
	if changed {
	...
	for {
		for _, threadByte := range threadBytes {
			...
			if hasNumZeroesSuffix(hashStrBuf.Bytes(), args.NumTrailingZeros) {
				...
				result := WorkerResult{args.Nonce, args.NumTrailingZeros, args.WorkerByte, secret, nil}
				trace.RecordAction(result)

				existed, changed := w.cache.Add(CacheEntry{args.Nonce, args.NumTrailingZeros, secret })
				if existed != nil {
					trace.RecordAction(CacheRemove{existed.Nonce, existed.NumTrailingZeros, existed.Secret})
				}
				if changed {
					trace.RecordAction(CacheAdd{args.Nonce, args.NumTrailingZeros, secret})
				}
